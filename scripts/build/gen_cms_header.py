...
from devicetree.edtlib import EDTError
...
def main():
    ...
    # Obtain list of nodes that are to be initialized
    # in a specific order. Note that the property must
    # be placed on a child node of the tree root to
    # avoid circular dependency errors.
    try:
        holder_node = edt.get_node(ORDERED_INIT_NODE_PATH)
        ordered_init_list_prop = holder_node.props.get(ORDERED_INIT_PROPERTY_NAME)
        if ordered_init_list_prop is not None:
            # Ordered initialization list has been provided.
            # Check that all nodes in the list were going to be
            # statically initialized, then move them from the
            # normal list to a special one we write out first.
            for node in ordered_init_list_prop.val:
                # if node is disabled, ignore it
                if not node.status == "okay":
                    continue

                # TODO: really panic?
                # or should we panic only if node isn't in clock tree?
                if not node in static_init_nodes:
                    print(f"{node.path} in ordered init list but not statically initialized!")
                    return -1

                static_init_nodes.remove(node)
                ordered_init_nodes.append(node)
    except EDTError:
        pass
...
""".replace(4 * " ", "\t").rstrip()
            ...
            if len(ordered_init_nodes) > 0:
                init_fn_code += "/* The following nodes are ordered according\n"
                init_fn_code += " * to the initialization list provided in DTS\n"
                init_fn_code += " */\n"
                for node in ordered_init_nodes:
                ...
                init_fn_code += "\n"
                init_fn_code += "/**********************************************************/\n"
                init_fn_code += "/* All other nodes are initialized in DT dependency order */\n"
                init_fn_code += "/**********************************************************/\n\n"
