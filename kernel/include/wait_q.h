...
/*
 * There is no "safe" variant of RB_FOR_EACH() macros so we cannot
 * provide _WAIT_Q_FOR_EACH_SAFE when CONFIG_WAITQ_SCALABLE=y.
 */
 ...
 * Walk all threads pending on a wait queue.
 *
 * @note The iterator thread may be safely detached from the wait queue
 * as part of the loop when using this macro.
 *
 * @param wq Wait queue
 * @param thread_ptr Thread pointer iterator variable
 * @param loop_ptr Thread pointer variable used by loop
