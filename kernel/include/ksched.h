 ...
 * @param data   `data` parameter of z_sched_waitq_walk()
 */
typedef void (*_waitq_walk_end_cb_t)(int status, void *data);
 ...
 * @param walk_func Callback to invoke on each waiting thread
...
int z_sched_waitq_walk(_wait_q_t *wait_q, _waitq_walk_cb_t walk_func,
		       _waitq_walk_end_cb_t post_func, void *data);
