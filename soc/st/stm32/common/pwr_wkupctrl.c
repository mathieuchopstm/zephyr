...
static bool is_wake_up_line_flag_active(uint8_t wkup_line_idx)
{
	...
	const uint32_t wuf = (stm32_reg_read(&PWR->WUCR) >> PWR_WUSR_WUF1_Pos);
...
int stm32_pwrc_dispatch_wakeup_gpio_irqs(void)
{
	...
	for (uint8_t line_idx = 1; line_idx <= MAX_WKUP_LINES; line_idx++) {
		...
		if (!ll_pwr_is_wake_up_line_flag_active(ll_line)) {
			continue;
		}
		...
		if (gpio_port == NULL) {
			LOG_ERR("GPIO port for active wake-up line (P%c%d) not found?!",
				'A' + pin_desc->port_idx, pin_desc->pin);
			continue;
		}

		gpio_data = gpio_port->data;
		gpio_fire_callbacks(&gpio_data->cb, gpio_port, BIT(pin_desc->pin));
