...
static void acquire_rng(void)
{
	...
	if (old_use_count == 0) {
		...
		/*
		 * If use count is non-zero, we should be owners of
		 * the RNG HSEM should and the TRNG should be enabled.
		 */
		ASSERT_RNG_HSEM_OWNED();
					     ...
					     uint32_t flags)
{
	...
	if (len) {
		 ...
		 * as soon as possible!
		 *
		 * It's unnecessary to disable interrputs while doing this because
