				   ...
				   bool enable)
{
	...
	if (isochronous) {
		...
		uint32_t bufs = ((uint32_t)buf2_pma_addr << 16) | buf1_pma_addr;

		hal_status = HAL_PCDEx_PMAConfig(&priv->pcd, ep_cfg->addr, PCD_DBL_BUF, bufs);
	} else {
		hal_status = HAL_PCDEx_PMAConfig(&priv->pcd, ep_cfg->addr, PCD_SNG_BUF, priv->occupied_mem);
	}
...
static void udc_stm32_init_ep_addr_caps(struct udc_ep_config *ep_cfg, uint8_t ep_addr)
{
	...
	if (ep_idx == 0u) {
		 ...
		 */
		const uint32_t num_iso_out =
			COND_CODE_1(IS_ENABLED(CONFIG_UDC_STM32_STUSB_ISOCHRONOUS_SUPPORT),
				(CONFIG_UDC_STM32_STUSB_ISO_OUT_EP_NUM), (0));
		const uint32_t num_iso_in =
			COND_CODE_1(IS_ENABLED(CONFIG_UDC_STM32_STUSB_ISOCHRONOUS_SUPPORT),
				(CONFIG_UDC_STM32_STUSB_ISO_IN_EP_NUM), (0));
