...
struct udc_stm32_data  {
	 ...
	 * buffered by driver but not acknowledged
...
// UTILITY FUNCTION, SHOULD MOVE TO UDC COMMON
			...
			struct net_buf *buf)
{
	...
	if (ep_cfg->addr == USB_CONTROL_EP_IN && len > 0U) {
		...
		//TBC:
		// On DWC2, it seems we used to enqueue a ZLP on OUT EP.
		// Replacing this with a mere HAL_PCD_EP_Receive(NULL, 0)
		// seems to work fine, but is it really sound?
...
static void handle_msg_data_out(struct udc_stm32_data *priv, uint8_t epnum, uint16_t rx_count)
{
	...
	if (ep == USB_CONTROL_EP_OUT) {
		...
		if (buf->len < priv->ep0_out_wlength) {
			HAL_StatusTypeDef __maybe_unused status;
			uint32_t rx_size = MIN(net_buf_tailroom(buf), UDC_STM32_EP0_MAX_PACKET_SIZE);
	...
	/* Buffer is full - remove it from queue and submit to stack */
	...
	} else { /* nothing left to do; EP is no longer busy */
		udc_ep_set_busy(ep_cfg, false);
	}
}
...
static void handle_msg_data_in(struct udc_stm32_data *priv, uint8_t epnum)
{
	...
	/* drop reference to packet & submit */
	udc_buf_get(ep_cfg);
	udc_submit_ep_event(dev, buf, 0);
...
static int submit_setup(struct udc_stm32_data *priv, struct net_buf *buf)
{
	/* HAL copies SETUP packet contents to priv->pcd.Setup */
...
static void handle_msg_setup(struct udc_stm32_data *priv)
{
	...
	if (buf == NULL) {
		/*
		 * Stack not ready to process SETUP packet.
		 * Remember we have one pending, we'll give
		 * it to the stack on request in ep_enqueue()
		 */
		priv->pending_setup = true;
	} else {
		submit_setup(priv, buf);
	}
}
