...
/*
 * Use a permanently upcounting timer @ 16 MHz for main timekeeping.
 * (NOTE: overflows every ~268 seconds or ~4 min 30 s - what to do?)
 *
 * When USB SOF occurs, capture the timer's value (in software).
 *
 * When an I2S frame is being shifted out, also capture the timer's
 * value, then reset and start a second 16 MHz timer until the
 * transfer ends, such that I2S_frame_timestamp is equal to captured
 * I2S TIM1 value + TIM2 counter value.
 */
...
BUILD_ASSERT(!IS_ENABLED(CONFIG_APP_USE_I2S_LRCLK_EDGES_COUNTER),
	"I2S LRCLK capture not supported yet");
...
#define MAIN_TIMER	TIM2	/* must be 32-bit */
#define I2C_TIMER	TIM3	/* can be 16-bit */
...
{
	/*
	 * This function is called from the SOF callback.
	 * Trigger timer SoF capture and get its value
	 * (somewhat inaccurate due to software delay...)
	 *
	 */
